...
var lunrExports = {};
var lunr$1 = {
  get exports(){ return lunrExports; },
  set exports(v){ lunrExports = v; },
};
...
var lunr = lunrExports;

async function makeIdx() {
    if (!window.idx) {
        window.index = await fetch(`${window.siteUrl}/index.json`).then((response) => response.json());
...
function SearchModal() {
    ...
    return {
        ...
        searching: false,
        empty: false,
        search: lodashExports.debounce(function (searchTerm) {
            this.searching = true;
            this.empty = false;
...
export { SearchModal, SearchResult };
